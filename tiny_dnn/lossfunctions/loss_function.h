...
class cross_entropy_multiclass {
 public:
  ...
  static half f(const vec16_t &y, const vec16_t &t) {
    assert(y.size() == t.size());
    half d{0.0};
  ...
  static vec16_t df(const vec16_t &y, const vec16_t &t) {
    assert(y.size() == t.size());
    half epsilon = (half)(1e-6); // small positive value to avoid log zero
    vec16_t d(t.size());
...
template <typename E>
vec16_t gradient(const vec16_t &y, const vec16_t &t) {
  assert(y.size() == t.size());
  return E::df(y, t);
}
...
template <typename E>
std::vector<vec16_t> gradient(const std::vector<vec16_t> &y,
                              const std::vector<vec16_t> &t) {
  std::vector<vec16_t> grads(y.size());
...
inline void apply_cost_if_defined(std::vector<vec16_t> &sample_gradient,
                                  const std::vector<vec16_t> &sample_cost) {
  if (sample_gradient.size() == sample_cost.size()) {
    // @todo consider adding parallelism
    const size_t channel_count = sample_gradient.size();
    for (size_t channel = 0; channel < channel_count; ++channel) {
...
template <typename E>
std::vector<tensor16_t> gradient(const std::vector<tensor16_t> &y,
                               const std::vector<tensor16_t> &t,
                               const std::vector<tensor16_t> &t_cost) {
  const size_t sample_count  = y.size();
  const size_t channel_count = y[0].size();

  std::vector<tensor16_t> gradients(sample_count);
